				  ...
				  float viewport_height)
{
	...
	cam->viewplane_height = (float)2.0/cam->aspect_ratio;

	// now we know fov and we know the viewplane dimensions plug into formula and
	// solve for view distance parameters
	float tan_fov_div2 = (float)tan(DEG_TO_RAD(fov/2));

	cam->view_dist = (float)(0.5)*(cam->viewplane_width)*tan_fov_div2;
	...
	else 
	{
		...
		VECTOR3D_INITXYZ(&vn,cam->view_dist,0,(float)-cam->viewplane_width/2.0); 
		PLANE3D_Init(&cam->rt_clip_plane, &pt_origin,  &vn, 1);
		...
		VECTOR3D_INITXYZ(&vn,0,cam->view_dist,(float)-cam->viewplane_width/2.0); 
		PLANE3D_Init(&cam->tp_clip_plane, &pt_origin,  &vn, 1);
	...
	case TRANSFORM_LOCAL_ONLY:
		{
			for (int poly = 0; poly < rend_list->num_polys; poly++)
			{
				...
				if ((curr_poly==NULL) || !(curr_poly->state & CW_POLY4DV1_STATE_ACTIVE) ||
					(curr_poly->state & CW_POLY4DV1_STATE_CLIPPED ) ||
					(curr_poly->state & CW_POLY4DV1_STATE_BACKFACE) )
...
void Draw_RENDERLIST4DV1_Wire32Bit(RENDERLIST4DV1_PTR rend_list,CWDraw & oWDraw)
{
	...
	for (int poly=0; poly < rend_list->num_polys; poly++)
	{
		...
		// irrelevant in a wire frame engine though
		if (!(rend_list->poly_ptrs[poly]->state & CW_POLY4DV1_STATE_ACTIVE) ||
			(rend_list->poly_ptrs[poly]->state & CW_POLY4DV1_STATE_CLIPPED ) ||
			(rend_list->poly_ptrs[poly]->state & CW_POLY4DV1_STATE_BACKFACE) )
			continue; // move onto next poly
		...
		oWDraw.DrawClipLine32Bit(rend_list->poly_ptrs[poly]->tvlist[0].x, 
		...
		oWDraw.DrawClipLine32Bit(rend_list->poly_ptrs[poly]->tvlist[1].x, 
			rend_list->poly_ptrs[poly]->tvlist[1].y,
			rend_list->poly_ptrs[poly]->tvlist[2].x, 
			rend_list->poly_ptrs[poly]->tvlist[2].y,
			rend_list->poly_ptrs[poly]->color);

		oWDraw.DrawClipLine32Bit(rend_list->poly_ptrs[poly]->tvlist[2].x, 
			rend_list->poly_ptrs[poly]->tvlist[2].y,
						...
						VECTOR4D_PTR rot)       // initial rotations
{
	...
	LONG lRetCode = 0;
	lRetCode = fopen_s(&fp,filename, "r");
	if (lRetCode!=0)
	{
		CWOOD_LOG_ERR_L(lRetCode,"Couldn't open PLG file %s.", filename);
		return lRetCode;
	} // end if
	...
	{
		CWOOD_LOG_ERR_L(-2,"PLG file error with file %s (object descriptor invalid).",filename);
		return(0);
	} // end if

	CWOOD_LOG_DEBUG("Object Descriptor: %s ", token_string);

	CWOOD_LOG_DEBUG("Object Descriptor: %s SizeTest:%d", token_string,sizeof(obj->name));

	// parse out the info object
	sscanf_s(token_string, "%s %d %d", obj->name,sizeof(obj->name), &obj->num_vertices, &obj->num_polys);

	CWOOD_LOG_DEBUG("Object Descriptor2: %s", token_string);
	...
	for (int vertex = 0; vertex < obj->num_vertices; vertex++)
	{
		...
		if (!(token_string = Get_Line_PLG(buffer, 255, fp)))
		{
			CWOOD_LOG_ERR_L(-4,"PLG file error with file %s (vertex list invalid).",filename);
			return(0);
		} // end if

		// parse out vertex
		sscanf_s(token_string, "%f %f %f", &obj->vlist_local[vertex].x, 
		...
		CWOOD_LOG_DEBUG("Vertex %d = %f, %f, %f, %f", vertex,
	...
	CWOOD_LOG_DEBUG("Object average radius = %f, max radius = %f", 
	...
	for (int poly=0; poly < obj->num_polys; poly++)
	{
		...
		if (!(token_string = Get_Line_PLG(buffer, 255, fp)))
		{
			CWOOD_LOG_ERR_L(-5,"PLG file error with file %s (polygon descriptor invalid).",filename);
			return(0);
		} // end if

		CWOOD_LOG_DEBUG("Polygon %d:", poly);
		...
		sscanf_s(token_string, "%s %d %d %d %d", tmp_string, sizeof(tmp_string),
		...
		CWOOD_LOG_DEBUG("Surface Desc = 0x%.4x, num_verts = %d, vert_indices [%d, %d, %d]", 
			poly_surface_desc, 
			poly_num_verts, 
			obj->plist[poly].vert[0],
			obj->plist[poly].vert[1],
		...
		if ((poly_surface_desc & PLX_COLOR_MODE_RGB_FLAG)) 
		{
			...
			int red   = (poly_surface_desc & 0xff0000);
			int green = (poly_surface_desc & 0x00ff00);
			int blue  = (poly_surface_desc & 0x0000ff);
			...
			obj->plist[poly].color = RGB32BIT(red, green, blue);
			CWOOD_LOG_DEBUG("RGB color = [%d, %d, %d]", red, green, blue);
		} // end if
		else
		{
			...
			CWOOD_LOG_DEBUG("8-bit color index = %d", obj->plist[poly].color);
		...
		obj->plist[poly].state = CW_POLY4DV1_STATE_ACTIVE;    
...
for (int poly = 0; poly < obj->num_polys; poly++)
    ...
    if (!(curr_poly->state & CW_POLY4DV1_STATE_ACTIVE))
       continue; // move onto next poly

    // reset clipped and backface flags
    RESET_BIT(curr_poly->state, CW_POLY4DV1_STATE_CLIPPED);
    RESET_BIT(curr_poly->state, CW_POLY4DV1_STATE_BACKFACE);
