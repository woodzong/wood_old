...
$g_aColor = array(0x0033CC,0x3300CC,0x9900CC,0xCC0099,0xCC0033,0xCC3300,0xCC9900,0x99CC00,0x33CC00,0x00CC33,0x00CC99,0x0099CC,0x0A47FF,0x4775FF,0xFFC20A,0xFFD147);
...
class CObjectXXOImg{
	...
	public function Relocation($iX,$iY){
		$this->m_iX = $iX;
		$this->m_iY = $iY;
	}
	
	public function RelocationOffSetX($iXOffset,$iYOffSet){
		$this->m_iX += $iXOffset;
		$this->m_iY += $iYOffSet;
	}
	...
	public function ShowOut($image){
		...
		imagecolordeallocate($image,$colorFilledColor);
		imagecolordeallocate($image,$colorBorderColor);
		imagecolordeallocate($image,$colorFontColor);
...
class CConnector{
	static public function ConnectAToB( $image, $oObjectXXOImg_A, $oObjectXXOImg_B ){
		...
		$colorLine = WCMakeColor($image,$g_aColor[$g_iLineColorIndex%count($g_aColor)]);
		...
		imagecolordeallocate($image,$colorLine);
	}
}
